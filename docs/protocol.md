...
| Payload Length | 4B | 上限 `16 MiB`。解码端读到超限值必须断开（防恶意长度导致 OOM）。 |
...
| 0x08 | `CANCEL` | 双向 | ≠0 | 取消流（半关闭语义见 §7.3） |
...
| 0x10 | `RESUME` | C→S | 0 | 断线重连：请求恢复会话 |
| 0x11 | `RESUME_OK` | S→C | 0 | 会话恢复成功，随后重放保留帧 |
| 0x12 | `RESUME_ERR` | S→C | 0 | 会话过期/未知，客户端必须重建全部状态 |
...
- **生效规则：以服务端配置为准**。服务端未启用压缩/加密而客户端声明了，按服务端 OFF 生效（降级而非报错）；客户端未声明而服务端要求加密，回 `ERROR(AUTH_DENIED)` 拒绝连接。每帧 Flags 如实标注本帧是否压缩/加密，**解码端按帧内 Flags 走管线，不依赖协商结果**——这样单连接内可以混合发送（例如心跳永远不压缩，大 payload 才压缩）。
...
```
   ...
   │      │
   │      └──────────→ CANCEL(收到) → CLOSED
   └─ CANCEL(发出)：半关闭，停止发送、继续接收直到对端 COMPLETE/ERROR
```
...
- **作用范围**：`Flags.Stream/Channel` 的下行 `RESPONSE`、订阅投递的下行 `PUBLISH`。请求/响应与 `ONEWAY` 单帧即终结，不纳入 credit（约束它们没有意义，反而增加一次 RTT）。
- **协商**：`CONNECT/CONNACK` 的 `credit` > 0 表示启用，双方按生效值执行；未启用时发送方无限制（诚实语义：关闭背压 = 信任 TCP 缓冲能兜住，内存风险自担）。
...
- 每个未终结流的**下行帧队列**（`RESPONSE`/`PUBLISH`/`SUBACK`/`COMPLETE`/`ERROR`/`CREDIT`，环形缓冲，默认 4 MiB/会话，超限则会话失去恢复资格）；
- 订阅关系（重连后继续投递）；
...
- 服务端校验会话存在且未过期 → `CONNACK{resumed:true}`，把新连接绑到旧会话，冻结中的队列解冻重放。
- 未过期但队列已溢出 → 同样 `resumed:true`，溢出流上补发 `ERROR(BUSY)`（客户端该流以错误收场，语义诚实：不假装没丢）。
- 会话过期/未知 → `RESUME_ERR`，客户端：所有挂起流以 `SESSION_EXPIRED` 失败、清空订阅表、回调 `OnResumeFailed` 让应用重建状态；随后重新走全新握手。

### 8.3 语义：at-least-once

重放机制不携带 per-frame 序号，客户端无法精确判重，因此恢复语义是 **at-least-once**：断连瞬间"已发出未收到"的帧会重发一次。选它的理由：JSON 业务消息绝大多数天然可幂等（携带业务 ID 可去重），而精确一次（exactly-once）需要 per-stream 序号 + 双端确认窗口，复杂度堪比一个 TCP。RSocket 同样提供 `resumable`（精确）与简化两种模式，我们把简化模式作为 v1 的唯一实现，序号扩展点留在 §10。
