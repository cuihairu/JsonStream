...
**类型字段**解决的是复用问题：一条 TCP 连接上要同时跑心跳、握手、多种交互模式的帧。没有类型，接收端无法区分"这 20 字节是心跳还是半个请求"。WebSocket 用 opcode、RSocket 用 frame type、MQTT 用 packet type——都是同一个答案：**帧类型是连接复用的最小前提**。本协议显式列出全部类型（§4），未知类型一律立即断开且不回帧（连帧类型都不认识，说明对端是异版本实现，回帧的互通前提已不成立），为将来升级留出"新类型 = 新能力"的演进路径。
...
收到任何未定义类型：立即断开且不回帧——连帧类型都不认识，说明对端是异版本实现，互通前提已不成立（对照 WebSocket 对未知 opcode 直接 fail the connection）。
...
S→C  CONNACK  {session_id, resumed, …生效参数}
     或 ERROR(AUTH_DENIED / PROTOCOL) 后断开
```
...
- 协议层**保证不产生任何响应帧**，包括错误（路由不存在也静默丢弃——不留日志是刻意的：ONEWAY 是高频路径，对端可用随机路由刷爆日志，等于把 DoS 面开进可观测性里）。一旦回帧，"无需返回"的承诺就被打破了——这与 RSocket `REQUEST_FNF` 的取舍相同：用"不可知送达"换"零等待、零反向流量"。
- 需要确认的场景请用请求/响应。
...
| 4 | `PROTOCOL` | 协议违规（坏魔数、未知类型等帧层错误；Flags 声明与 handler 模式不符也用它，但按语义错误只终结流，见下方惯例） |
...
| 9 | `BUSY` | 过载拒绝 |
| 10 | `UNSUPPORTED` | 对端使用了本端未启用的能力（如本端未启用加密却收到加密帧）；发生在读管线变换层，payload 解不出来，接收方直接断连 |

惯例：断连与否由错误发生在**哪一层**决定——**握手与帧/变换层错误必须断开**（版本不匹配、坏魔数、未知类型、非零保留位、解密/解压失败、`UNSUPPORTED`：此时 payload 根本解不出来，无流可终结，状态已不可信）；**分发层语义错误只终结所在流**：应用错误（1/2/3/6/9）以 `ERROR` 回执后连接继续服务其他流；请求的 Flags 声明与路由注册的 handler 类型不符回 `ERROR(PROTOCOL)` 同样只终结该流——一个路由打错不应杀掉全连接。
