...
**分解出的固定成本 vs 边际成本**（本机实测，见 §11 表格）：小帧 ~2.4µs/帧的固定开销、~0.8–1.1 ns/B 的边际开销，两者在 L ≈ 2–3KiB 附近交叉。也就是说：
...
心稳态成本是每 `Heartbeat` 一次 14B 写；读侧每帧一次 `SetReadDeadline`（netpoll 的 deadline 表更新，纳秒级）。两者都远小于一次 RTT，所以"心跳开着"不会成为吞吐瓶颈——这是把心跳做成"每连接独立 ticker"而不是"全局扫描"的理由。
...
下表是**本机实测**（`go test -run '^$' -bench . -benchtime 1s -benchmem`，共享容器环境，与 README 表格的 i9-10880H 不是同一台机器，只有量级与分配次数可比）：

| 基准 | ns/op | 吞吐 | allocs/op | B/op |
| --- | --- | --- | --- | --- |
| 帧往返 64B | 2 431 | 42.8 MB/s | 6 | 216 |
| 帧往返 1KiB | 3 473 | 306 MB/s | 6 | 1 176 |
| 帧往返 64KiB | 54 289 | 1 208 MB/s | 6 | 65 688 |
| 变换·明文（1.3KiB） | 30 | — | **0** | 0 |
| 变换·flate | 53 149 | — | 8 | 5 186 |
| 变换·AES-GCM | 7 453 | — | 3 | 3 088 |
| 变换·压+加 | 52 972 | — | 11 | 5 755 |
| 请求/响应 RTT（明文） | 196 857 | — | 33 | 2 130 |
| 请求/响应 RTT（加密） | 158 733 | — | 39 | 2 330 |

从这组数字能直接读出三条结论（都已写进 §9 的取舍）：

1. **allocs/op 与载荷大小无关**（三种尺寸都是 6）——说明帧头的定长数组与元数据解码没有引入按帧的隐藏分配；大帧的 B/op 线性增长全部来自载荷本身。
2. **flate 比 GCM 贵约 7 倍**（53µs vs 7.5µs，同一载荷）——所以"压缩按帧可选、加密可全开"的参数化策略在性能上是有依据的取舍，而不是口号。
3. **小帧的固定成本主导**：64B→1KiB 只多 1µs（1KiB 载荷本身只值 ~1µs 内存操作），说明小消息场景的优化方向是减少帧数与 syscall，不是压载荷。
