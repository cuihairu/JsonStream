...
**这一步每轮到底搜了多少空间**（i9-10880H、12 fuzz worker、20s 预算，取 `-fuzz` 自报的 execs 计数）：

| 靶 | 20s 内 execs | 聚合速率 | 每 exec 的主导成本 |
| --- | --- | --- | --- |
| `FuzzReadFrame` | 17.4 万 | ~8k/s | 纯内存帧解析，最便宜 |
| `FuzzTransformInbound` | 23.2 万 | ~11k/s | 每 exec 一次 flate + AES-GCM，单价最贵却最高产 |
| `FuzzRawPeer` | 3.7 万 | ~1.7k/s | 每 exec 起一条真实 TCP 连接走完握手，进程创建开销主导 |
| 合计 | ~44 万 | ~7.4k/s | 单次 CI ≈ 44 万次变异（CI runner 核数更少，实际更低） |
