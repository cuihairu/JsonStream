...
**fuzz 的两种用法别搞混**：种子语料随 `go test` 常规跑（CI 兜底，crash 语料自动落进 `testdata/fuzz/` 成为永久回归用例），`-fuzz` 是随机挖掘（本地深挖，CI 里也跑一层固定预算的冒烟——靶名靠 `go test -list` 发现、每靶 20s，细节见 DESIGN §11.4）。真正的防线是把不变量写成断言（"任意字节不 panic""parse∘encode = 恒等"），fuzz 引擎只提供搜索力——**约束归断言，工具只负责找反例**。所以覆盖率或 fuzz 时长都不是"安全"的度量，只是搜索预算。
